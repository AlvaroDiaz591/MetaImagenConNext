import type { AppointmentCreateRequest, PatientAppointmentSummary } from "../../domain/entities/PatientAgenda";
...
export class CreatePatientAppointmentUseCase {
  ...
  async execute(params: AgendaAuthParams & { payload: AppointmentCreateRequest }): Promise<PatientAppointmentSummary> {
    if (!params.token.trim()) {
      throw new Error("No se encontro un token de sesion valido.");
    }
    return this.service.crearCita(params);
  }
}
